																												...
																												enabled_logging(enabled_logging), logging_period(logging_period), logging_file_path(logging_file_path)
{
    ...
    uint32_t IO_Flow_Base::Get_device_read_response_time()
    {
    ...
    uint32_t IO_Flow_Base::Get_device_write_response_time()
    {
	...
	void IO_Flow_Base::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
	{
		...
		attr = "IOPS";
		val = std::to_string((double)STAT_generated_request_count / ((double)(Simulator->Time() - Simulator->running_milestone) / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "IOPS_Read";
		val = std::to_string((double)STAT_generated_read_request_count / ((double)(Simulator->Time() - Simulator->running_milestone) / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "IOPS_Write";
		val = std::to_string((double)STAT_generated_write_request_count / ((double)(Simulator->Time() - Simulator->running_milestone) / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);
		...
		attr = "Bandwidth";
		val = std::to_string((double)STAT_transferred_bytes_total / ((double)(Simulator->Time() - Simulator->running_milestone) / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Bandwidth_Read";
		val = std::to_string((double)STAT_transferred_bytes_read / ((double)(Simulator->Time() - Simulator->running_milestone) / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Bandwidth_Write";
		val = std::to_string((double)STAT_transferred_bytes_write / ((double)(Simulator->Time() - Simulator->running_milestone) / SIM_TIME_TO_SECONDS_COEFF));
		xmlwriter.Write_attribute_string(attr, val);
		...
		val = std::to_string(Get_device_read_response_time());
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Write_Device_Response_Time";
		val = std::to_string(Get_device_write_response_time());
		xmlwriter.Write_attribute_string(attr, val);
