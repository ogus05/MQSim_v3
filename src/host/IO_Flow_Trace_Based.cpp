...
void IO_Flow_Trace_Based::Start_simulation()
{
	...
	if(!load_trace_file.is_open()){
		...
		while (std::getline(load_trace_file, trace_line))
		{
			...
			{
				PRINT_ERROR("Unexpected request arrival time in load phase file: " << last_request_arrival_time_1 << "\nMQSim expects request arrival times to be monotonically increasing in the input trace!")
			}
		}

		load_trace_file.close();
		PRINT_MESSAGE("Load trace file: " << load_trace_file_path << " seems healthy");
	}
	...
	while (std::getline(trace_file, trace_line))
	{
		...
		{
			PRINT_ERROR("Unexpected request arrival time in run phase file: " << last_request_arrival_time_2 << "\nMQSim expects request arrival times to be monotonically increasing in the input trace!")
		}
	}
...
void IO_Flow_Trace_Based::Execute_simulator_event(MQSimEngine::Sim_Event *)
{
	...
	if (STAT_generated_request_count < total_requests_to_be_generated)
	{
		...
		std::string trace_line;
		if (std::getline(*curTraceFile, trace_line))
		{
			Utils::Helper_Functions::Remove_cr(trace_line);
			current_trace_line.clear();
			Utils::Helper_Functions::Tokenize(trace_line, ASCIILineDelimiter, current_trace_line);
		...
		else
		{
			if(loadPhase){
				...
				trace_file.open(trace_file_path);
				curTraceFile = &trace_file;
				std::getline(*curTraceFile, trace_line);
				Utils::Helper_Functions::Remove_cr(trace_line);
				current_trace_line.clear();
				Utils::Helper_Functions::Tokenize(trace_line, ASCIILineDelimiter, current_trace_line);
				char *pEnd;
				Simulator->Finish_LoadPhase(std::strtoll(current_trace_line[ASCIITraceTimeColumn].c_str(), &pEnd, 10), this);
				return;
			} else{
				curTraceFile->close();
				trace_file.open(trace_file_path);
		...
		Simulator->Register_sim_event(Simulator->loadMileStone + time_offset + std::strtoll(current_trace_line[ASCIITraceTimeColumn].c_str(), &pEnd, 10), this);
	}
}
